...
app.listen(3005, () => {
  console.log("Hey , yoour server is running on port 3005!");
});
...
app.get("/customerData/:phone/:plate", (req, res) => {
  ...
  db.query(phoneSql, [phone], (phoneErr, phoneResult) => {
    if (phoneErr) {
      ...
      res.status(500).json({ error: 'เกิดข้อผิดพลาดในการค้นหาหมายเลขโทรศัพท์' });
      ...
      db.query(plateSql, [plate], (plateErr, plateResult) => {
        if (plateErr) {
          ...
          res.status(500).json({ error: 'เกิดข้อผิดพลาดในการค้นหาหมายเลขทะเบียนรถ' });
          ...
          db.query(
            ...
            (err, result) => {
              if (err) {
                console.log(err);
                res.status(500).json({ error: 'เกิดข้อผิดพลาดในการค้นหาข้อมูลรายการซ่อม' });
              } else {
                res.json(result);
              }
            }
